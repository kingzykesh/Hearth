...
export default function AdminResearchPage() {
  ...
  const fetchStats = async () => {
    try {
      setLoading(true);
      const response = await api.get("/api/admin/research/stats");
  ...
  return (
    <section className="space-y-6">
        ...
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-7 text-white/65">
              Real student wellness labels plus voice signals powering your
      ...
      <div className="rounded-[30px] border border-white/10 bg-white/[0.04] p-7">
        <h3 className="text-xl font-bold text-white">Dataset Moat Status</h3>

        <div className="mt-5 grid gap-5 md:grid-cols-3">
          <InsightBox
            title="Moat"
            text="You are building a proprietary African wellness voice dataset that public datasets do not provide."
          />
          <InsightBox
            title="Accuracy"
            text="Future Hearth models can learn from real Nigerian student wellness labels instead of only actor-emotion datasets."
          />
          <InsightBox
            title="Investor Signal"
            text="Data ownership, recurring usage, and local distribution create defensibility for Hearth Africa Model v1."
          />
        </div>
      </div>
    </section>
